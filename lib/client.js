    ...
    function createClientPlugin() {
      return {
        ...
        apply(ctx) {
           ...
           * 默认 **false** —— 启动时保持 DSH 原生列序，只有点了右上角那枚按钮才
           * 交换；有意不做持久化，所以每次启动都从原生状态开始。
           */
          let swapped = false
          ...
          function sync() {
            ...
            if (!announced) {
              announced = true
              console.log('[dsh-column-swap] ready — rail/sidebar =', railWidth, '/', rightbarWidth,
                swapped ? '(swapped — click the header button to restore)' : '(native order — click the header button to swap)')
            }
          }
