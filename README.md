...
> 把 DSH 的**原生右侧栏**与**智能体对话栏**对调位置：`[左栏 | 对话栏 | 右列]` → `[左栏 | 右列 | 对话栏]`，两侧宽度各自保留。**启动时保持原生列序**（不改变任何布局），点窗口右上角那枚「两列 + ⇄」按钮才交换、再点切回；状态不做持久化，每次启动都从原生开始。
...
点击按钮交换后，某次实测（视口 1854×1044、右列宽 820px）：
...
6. **总开关**：所有布局规则都挂在 `[data-dsh-swap-on]` 门下；开关只加/去这一个属性，CSS 变量与列标记都保留着，所以切换即时生效、无需重算。按钮注册进 `conversation.session.header.utilities`（**list** 槽，纯追加、右对齐）—— 单个 `conversation.session.header.corner` 归 DSH 原生右侧栏包所有，不能抢；而交换态下对话栏就是最右列，这一行正好是窗口右上角，紧贴原生的右侧栏展开按钮。按钮样式只用产品已有的 token，文案跟随 `navigator.language`。**默认不交换**：插件启动时保持 DSH 原生列序，按钮是唯一的切换入口，且这个选择有意不做持久化 —— 每次启动都从原生状态开始。
...
- **交换 / 恢复**：点右上角按钮即可来回切换 —— 缺省即原生列序，按钮不持久化状态（不改文件、不重启）。
